package org.example.financetrackerapi.transaction.repository;
...
""")
    ...
    @Query(value = """
            SELECT new org.example.financetrackerapi.transaction.dto.CategorySummaryResponse(t.category.name,SUM(t.amount)
            )
            FROM Transaction t
            WHERE t.account.user = :user
            AND t.type = 'DEBIT'
...
"""
    ...
    @Query(
            ...
            SELECT COALESCE(SUM(
            CASE 
               WHEN t.type = org.example.financetrackerapi.transaction.enums.TransactionType.CREDIT
               THEN t.amount
               ELSE 0
               END
            ),0) 
            ...
            COALESCE(SUM( 
            CASE 
                WHEN t.type = org.example.financetrackerapi.transaction.enums.TransactionType.DEBIT
                THEN t.amount
                ELSE 0 
                END 
                ),0)
